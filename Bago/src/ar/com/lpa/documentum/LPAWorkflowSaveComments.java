...
public class LPAWorkflowSaveComments implements IDmMethod {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			IDfPackage paquete = workitem.getPackage(ServiceBPM
					.getIdPackageDoc());

			int cant_comentarios = paquete.getNoteCount();
			LPATDAWorkflowComments comentario = new LPATDAWorkflowComments();
			if (debug)
				logger.write("recorre los comentarios");
			for (int i = 0; i < cant_comentarios; i++) {
				if (debug)
					logger.write("comentario " + i);
				if (debug)
					logger.write("comentario id: "
							+ paquete.getNoteId(i).toString());
				comentario.setNombreWorkflow(process.getObjectName());
				comentario.setIdDocumento(dmcDoc.getObjectId().toString());
				comentario.setCodDocumento(dmcDoc.getString(bof
						.getPropiedadCodigo()));
				comentario.setVersion(dmcDoc.getString(bof
						.getPropiedadVersion()));
				comentario.setIdComentario(paquete.getNoteId(i).toString());
				DB.insertWorkflowComments(comentario);
				if (debug)
					logger.write("comentario insertado");
			}
			workitem.complete();
			if (debug)
