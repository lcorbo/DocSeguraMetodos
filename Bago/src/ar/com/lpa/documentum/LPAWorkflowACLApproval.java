...
public class LPAWorkflowACLApproval
  ...
  private void modificarACL(LPAServiceDataBase DB, IDfSysObject doc) throws Exception
  {
    try {
      ...
      if (acl.getObjectName().startsWith("acl_")) {
        ...
        String aprobador = doc.getAllRepeatingStrings("p_aprobador", ";");
        StringTokenizer st2 = new StringTokenizer(aprobador, ";");
        while (st2.hasMoreTokens()) {
          String grupo = (String)st2.nextElement();
          acl.grant(grupo, 6, "EXECUTE_PROC");
        }
        if (bof.getPropiedadSector() != null) {
          acl.grant(doc.getString(bof.getPropiedadSector()), 3, 
            "EXECUTE_PROC");
        }
        acl.grant("ac_aprob", 3, "EXECUTE_PROC");
        ...
        String grupos = doc
          .getAllRepeatingStrings("p_aprobadores", ";");
        StringTokenizer st = new StringTokenizer(grupos, ";");
        while (st.hasMoreTokens()) {
          String grupo = (String)st.nextElement();
          acl.grant(grupo, 6, "EXECUTE_PROC");
