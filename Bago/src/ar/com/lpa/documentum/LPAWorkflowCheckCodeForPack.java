...
public class LPAWorkflowCheckCodeForPack implements IDmMethod {
	...
	private boolean chequeaCodigo(IDfSysObject dmcDoc, LPATDABOF BOF)
			throws Exception {
		boolean valido = true;
		IDfCollection col = null;
		try {
			IDfQuery cons = new DfQuery();
			cons.setDQL("select count(*) as cantidad from dm_dbo.dm_sysobject_s, dm_dbo.dm_sysobject_r, dm_dbo.dm_policy_r, "
					+ dmcDoc.getTypeName()
					+ " where "
					+ dmcDoc.getTypeName()
					+ "."
					...
					+ dmcDoc.getTypeName()
					+ ".r_object_id and dm_dbo.dm_sysobject_s.r_object_id = dm_dbo.dm_sysobject_r.r_object_id and dm_dbo.dm_sysobject_r.r_version_label like '%CURRENT%' and dm_dbo.dm_policy_r.i_state_no = dm_dbo.dm_sysobject_s.r_current_state and dm_dbo.dm_policy_r.state_name <> 'No_Vigente'");
			if (debug)
				logger.write("DQL: "
						+ cons.getDQL());
			col = cons.execute(session, IDfQuery.DF_QUERY);
			if (col.next()) {
				if (col.getInt("cantidad") > 1)
					valido = false;
			}
			col.close();
		} catch (Exception e) {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			boolean valido = chequeaCodigo(dmcDoc, BOF);
			dmcBPM.setBoolean("bpm_check_codigo", valido);
			dmcBPM.save();
