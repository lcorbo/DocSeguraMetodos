...
public class LPAWorkflowChangeACL
    ...
    try {
      ...
      if (acl.getObjectName().startsWith("acl_")) {
        ...
        while (st.hasMoreTokens()) {
          ...
          acl.grant(grupo, 
            RE.getAclPermit("ChangeACL_destinatarios"), 
            RE.getAclPermitEx("ChangeACL_destinatariosEx"));
        }
        String[] gruposExtras = bof.getGruposExtras();
        if (gruposExtras != null) {
          for (int j = 0; j < gruposExtras.length; j++) {
            acl.grant(gruposExtras[j], 
              RE.getAclPermit("ChangeACL_grupos_extra"), 
              RE.getAclPermitEx("ChangeACL_grupos_extraEx"));
          }
        }
        if (bof.getPropiedadSector() != null) {
          acl.grant(doc.getString(bof.getPropiedadSector()), 
            RE.getAclPermit("ChangeACL_sector"), 
            RE.getAclPermitEx("ChangeACL_sectorEx"));
          acl.grant(
            doc.getString(bof.getPropiedadSector()) + "-edit", 
            RE.getAclPermit("ChangeACL_sector_editores"), 
            RE.getAclPermitEx("ChangeACL_sector_editoresEx"));
        }
        acl.grant(this.m_userName, 
          RE.getAclPermit("ChangeACL_Administrator"), 
          RE.getAclPermitEx("ChangeACL_AdministratorEx"));
        acl.grant("dm_world", RE.getAclPermit("ChangeACL_world"), 
          RE.getAclPermitEx("ChangeACL_worldEx"));
        acl.grant("dm_owner", RE.getAclPermit("ChangeACL_owner"), 
          RE.getAclPermitEx("ChangeACL_ownerEx"));
        acl.save();
        if (debug)
