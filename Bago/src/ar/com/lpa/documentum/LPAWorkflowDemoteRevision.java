...
public class LPAWorkflowDemoteRevision implements IDmMethod {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			do{
			dmcDoc.demote(dmcDoc.getPreviousStateName(), false);
			
			if (debug)
				logger.write("Demote realizado: "+dmcDoc.getCurrentStateName());
			
			}while(dmcDoc.getPreviousStateName().equalsIgnoreCase("PreEdicion")==false);
