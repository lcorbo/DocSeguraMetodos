...
public class LPAWorkflowSaveOrder implements IDmMethod {
	...
	private void insertaOrden(LPAServiceDataBase DB, LPATDAWorkflowBPM BPM,
			String nombreWorkflow, IDfSysObject dmcDoc) throws Exception {
		try {
			String propiedades = "";
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			insertaOrden(DB, BPM, workitem.getActivity().getObjectName(),
					dmcDoc);
			workitem.complete();
			if (debug)
