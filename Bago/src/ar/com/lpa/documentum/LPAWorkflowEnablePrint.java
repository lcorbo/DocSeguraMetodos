...
public class LPAWorkflowEnablePrint implements IDmMethod {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			IDfSysObject bpm = ServiceBPM.getParametros();
			LPATDABOF BOF = DB.getBOF(dmcDoc.getTypeName());
			DB.habilitaImpresion(dmcDoc.getString(BOF.getPropiedadCodigo()),
					bpm.getString("bpm_sector"));
			workitem.complete();
			if (debug)
