...
public class LPAWorkflowHerramientas implements IDmMethod {
	...
	private void insertaOrdenes(LPAServiceDataBase DB, IDfSysObject dmcDoc,
			LPATDABOF BOF, String workflow, String workflowID) throws Exception {
		try {
			String sectores = dmcDoc.getAllRepeatingStrings("p_auditados", ";");
			if (debug)
				logger.write("sectores: "
						+ sectores);
			StringTokenizer st = new StringTokenizer(sectores, ";");
			while (st.hasMoreTokens()) {
				String sector = (String) st.nextElement();
				String propiedades = workflowID + ";" + sector + ";" + sector;
				DB.insertWorkflowOrden(workflow, dmcDoc.getObjectId()
						.toString(), propiedades, "NO");
				dmcDoc.setInt("contador_subprocesos",
						dmcDoc.getInt("contador_subprocesos") + 1);
				dmcDoc.save();
			}
		} catch (Exception e) {
			throw e;
