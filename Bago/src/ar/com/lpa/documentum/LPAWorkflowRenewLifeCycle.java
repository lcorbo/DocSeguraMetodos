...
public class LPAWorkflowRenewLifeCycle implements IDmMethod {
	...
	private void renewLifeCycle(LPAServiceDataBase DB, IDfSysObject doc)
			throws Exception {
		try {
			LPATDABOF bof = DB.getBOF(doc.getTypeName());
			if (debug)
				logger.write("Comienza la renovacion de la ACL");
			doc.detachPolicy();
			String qualification = "dm_policy where object_name ='"
					+ bof.getLifeCycle() + "' and r_definition_state='2'";
			IDfId bpId = session.getIdByQualification(qualification);
			if (debug)
				logger.write("ID Ciclo de vida:    "
						+ bpId);
			if (bpId == null || bpId.toString() == "0000000000000000") {
				if (debug)
					logger.write("Ciclo de vida no encontrado.");
			} else {
				doc.attachPolicy(bpId, "0", "");
				doc.save();
				if (debug)
					logger.write("Ciclo de vida renovado");
			}
		} catch (Exception e) {
			throw e;
		}
