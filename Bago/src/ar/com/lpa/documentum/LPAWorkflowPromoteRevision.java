...
public class LPAWorkflowPromoteRevision implements IDmMethod {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			do{
			dmcDoc.promote(dmcDoc.getNextStateName(), true, false);
			if (debug)
				logger.write("Promote realizado: "+dmcDoc.getCurrentStateName());
			}while(dmcDoc.getNextStateName().equalsIgnoreCase("Vigente")==false);
			workitem.complete();
			if (debug)
