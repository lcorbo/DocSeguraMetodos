...
public class LPAWorkflowAddRevision implements IDmMethod {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			String revision = dmcDoc.getString(BOF.getPropiedadRevision());
			
			
				if (debug)
					logger.write("Revision: "
							+ revision);
				int majorRevision = Integer.parseInt(revision);
				majorRevision = majorRevision + 1;
				
				if (debug)
					logger.write("Revision: "
							+ majorRevision);
				dmcDoc.setInt(BOF.getPropiedadRevision(), majorRevision);
			
			dmcDoc.save();
			if (debug)
				logger.write("Revision aumentada");
