...
public class LPAWorkflowLoggerSector implements IDmMethod {
	...
	private void grabaLog(LPAServiceDataBase DB, IDfSysObject dmc, String paso,
			String workflowID, String workflowName, LPATDABOF bof, String sector)
			throws Exception {
		...
		try {
			...
			log.setMensaje(log.getMensaje().replace("SECTOR", sector));
			if (debug)
				logger.write("MensajeReemplazado: "
						+ log.getMensaje());
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			grabaLog(DB, dmcDoc, workitem.getActivity().getObjectName(),
					...
					parametros.getString("bpm_sector"));
