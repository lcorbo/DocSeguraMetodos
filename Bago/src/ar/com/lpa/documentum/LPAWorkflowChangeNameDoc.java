...
public class LPAWorkflowChangeNameDoc implements IDmMethod {
	...
	private void cambiaNombre(LPAServiceDataBase DB, IDfSysObject dmcDoc) throws Exception {
		try {
			LPATDANomenclature nomenclador = new LPATDANomenclature();
			nomenclador = DB.getNomenclature(dmcDoc.getTypeName());
			if (debug)
				logger.write("ChangeNameDoc - typename: " + dmcDoc.getTypeName());
			if (dmcDoc.getTypeName().equals("t_adjuntos")|| dmcDoc.getTypeName().equals("t_capacitacion")){
				if (debug)
					logger.write("ChangeNameDoc - VIA ADJ Y CAP");
				dmcDoc.setObjectName(dmcDoc.getString(nomenclador.getPropiedadCodigo()));
			}else{
				if (debug)
					logger.write("ChangeNameDoc - VIA NORMAL");
				dmcDoc.setObjectName(dmcDoc.getString(nomenclador.getPropiedadCodigo())	+ " - "	+ dmcDoc.getCurrentStateName());
			}
			dmcDoc.save();
		} catch (Exception e) {
			throw e;
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			cambiaNombre(DB, dmcDoc);
			if (debug)
				logger.write("Cambia el nombre del documento");
