...
public class LPAWorkflowChangeNameDocPreEdicion implements IDmMethod {
	...
	private void cambiaNombre(LPAServiceDataBase DB, IDfSysObject dmcDoc)
			throws Exception {
		try {
			LPATDANomenclature nomenclador = new LPATDANomenclature();
			nomenclador = DB.getNomenclature(dmcDoc.getTypeName());
			dmcDoc.setObjectName(dmcDoc.getString(nomenclador
					.getPropiedadCodigo())
					+ " - "
					+ "PreEdicion");
			dmcDoc.save();
