...
public class LPAWorkflowCheckSubProcess implements IDmMethod {
	...
	public void execute(Map params, OutputStream ostream) throws Exception {
		...
		try {
			...
			IDfSysObject parametros = ServiceBPM.getParametros();

			dmcDoc.setInt("contador_subprocesos",
					dmcDoc.getInt("contador_subprocesos") - 1);
			dmcDoc.save();

			if (dmcDoc.getInt("contador_subprocesos") == 0) {
				if (debug)
					logger.write("contador de subprocesos = 0");
				LPAServiceSubProcess subProcess = new LPAServiceSubProcess(
						session, logger, debug);
				subProcess.CompleteTask(props.getUsuarioEspera(),
						parametros.getString("bpm_workflow_padre"),
						"Espera SubProcesos");
			}
